...
| T-064 | Estandarizar catálogo de errores por dominio (`auth`, `tenant`, `membership`, etc.) con internacionalización de `clientMessage` por locale | `keygo-api`, `keygo-app` | Permite mensajes consistentes y localizables para clientes multi-idioma sin exponer detalles técnicos |
...
| T-047 | **SigningKeyInitializer** — `@Profile("supabase") ApplicationRunner` que genera un par RSA-2048 y lo persiste via `SigningKeyRepositoryPort` si no existe clave ACTIVE en startup; idempotente en re-inicios | 2026-03-24 | `SigningKeyInitializer.java` (nuevo en `keygo-run/startup/`) |
