...
| T-129 | **Consolidar lógica de generación en el dominio (Domain Services):** Lógica de negocio dispersa en ~15 use cases que debería vivir en `keygo-domain` como domain services puros. 5 factories: `CredentialFactory` (passwords, secrets, hashing), `VerificationCodeFactory` (códigos 6 dígitos), `TokenIdentifierFactory` (JTI, refresh tokens opacos), `InvoiceNumberGenerator`, `UsernameFactory` (+ resolución colisiones T-128). Principio: use cases orquestan, el dominio genera. | 🔲 Pendiente |
| T-130 | **Endpoint `POST /api/v1/platform/account/check-email`:** valida si un correo existe como `platform_user` antes de avanzar al paso de ToS en el hosted login. Público (sin Bearer), pero requiere sesión activa de `/oauth2/authorize` (atributo `platformAuthorizationState`). Respuestas: `200 PLATFORM_USER_EMAIL_FOUND` / `404 PLATFORM_USER_EMAIL_NOT_FOUND` / `401 AUTHENTICATION_REQUIRED`. Reutiliza `PlatformUserRepositoryPort.existsByEmail()`. Ver plan detallado: [`tasks/T-130-check-email.md`](tasks/T-130-check-email.md). | 🔲 Pendiente |
...
| T-126 | **Endpoints admin para gestión de status de PlatformUser:** `PUT /platform/users/{userId}/suspend`, `/activate`, `/require-reset-password`; 3 use cases; `@PreAuthorize("hasAuthority('ADMIN')")` — actualmente el status solo se gestiona vía DB directa | 🔲 Pendiente |
