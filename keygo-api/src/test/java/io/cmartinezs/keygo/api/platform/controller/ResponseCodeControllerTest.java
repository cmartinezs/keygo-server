...
import io.cmartinezs.keygo.api.shared.response.MessageResponse;
...
/**
 * Unit tests for ResponseCodeController
 * Pruebas unitarias para ResponseCodeController
 *
 * @author cmartinezs
 * @version 1.0
 */
...
class ResponseCodeControllerTest {
  ...
  void getResponseCodeCatalog_allCodes_shouldHaveCodeAndMessage() {
    ...
    catalog.getSuccessCodes().forEach(code -> {
    ...
    catalog.getFailureCodes().forEach(code -> {
      assertThat(code.getCode()).isNotNull().isNotEmpty();
      assertThat(code.getMessage()).isNotNull().isNotEmpty();
