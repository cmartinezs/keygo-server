...
| `ForgotPasswordUseCase.java` | `keygo-app` | Crear | (1) Resolver tenant; (2) buscar user por email → **si no existe: retornar `sent=true` sin error** (evitar oracle de enumeración); (3) verificar que user esté `ACTIVE` o `PENDING`; (4) generar token UUID; (5) persistir vía `upsert`; (6) `EmailNotificationPort.sendPasswordRecoveryEmail(...)` |
