...
sequenceDiagram
    ...
    KeyGo->>SPA: 4. Return:<br/>{<br/>  plans: [<br/>    { code: 'BASIC',<br/>      name: 'Basic Plan',<br/>      billingOptions: [<br/>        { period: MONTHLY, price: 9.99 },<br/>        { period: ANNUAL, price: 99.99 }<br/>      ],<br/>      entitlements: [<br/>        { feature: 'Users', limit: 5 }<br/>      ]<br/>    }<br/>  ]<br/>}
...
```mermaid
stateDiagram-v2
    [*] --> PENDING: Contract creado<br/>Invoice PENDING
    
    PENDING --> ACTIVE: Pago recibido<br/>Invoice status=PAID
    
    ACTIVE --> ACTIVE: Usuario disfruta plan<br/>Dentro currentPeriodEnd
    
    ACTIVE --> RENEWAL: currentPeriodEnd alcanzado<br/>(T-085: Job cron)
    
    RENEWAL --> ACTIVE: Auto-renew=true<br/>Nueva factura generada
    
    ACTIVE --> FAILED: Invoice pago fallido<br/>(T-090: Dunning)
    
    FAILED --> ACTIVE: Reintento exitoso<br/>(D+1, D+3, D+7)
    
    FAILED --> SUSPENDED: Dunning agotado<br/>Max reintentos alcanzado
    
    SUSPENDED --> CANCELLED: Usuario cancela<br/>o Admin termina
    
    CANCELLED --> [*]: Fin de suscripción<br/>Data histórica preservada
    
    ACTIVE --> CANCELLED: Usuario cancela<br/>(PUT .../cancel)
...
sequenceDiagram
    ...
    Admin->>API: 1. POST /api/v1/tenants/{slug}/apps/{appId}/billing/plans<br/>Header: Bearer {admin_token}<br/>body: {<br/>  code: 'BASIC',<br/>  name: 'Basic Plan',<br/>  billingOptions: [<br/>    { period: MONTHLY, basePrice: 9.99,<br/>      isDefault: true },<br/>    { period: ANNUAL, basePrice: 99.99 }<br/>  ],<br/>  entitlements: [<br/>    { feature: 'Users', limit: 5 },<br/>    { feature: 'Storage', limit: 1000 }<br/>  ]<br/>}
