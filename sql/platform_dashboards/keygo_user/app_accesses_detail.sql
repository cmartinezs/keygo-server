-- Purpose: drill-down app access activity for a platform user.
-- Parameters: :platform_user_id, :client_app_id, :from_ts, :to_ts, :limit, :offset
SELECT
    pae.occurred_at,
...
WHERE pae.platform_user_id = CAST(:platform_user_id AS uuid)
  AND pae.client_app_id = CAST(:client_app_id AS uuid)
