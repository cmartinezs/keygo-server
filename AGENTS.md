...
| Fecha | Cambio |
|---|---|
| 2026-04-02 | **Trazabilidad end-to-end:** `RequestTracingFilter` (MDC traceId/method/path + header `X-Trace-ID`); MDC enriquecido en `BootstrapAdminKeyFilter` (userId) y `TenantResolutionFilter` (tenantSlug); `traceId` en `ErrorData` + `ApiErrorDataFactory`; `logback-spring.xml` (consola colorida en `local`, JSON Logstash en `!local`); `logstash-logback-encoder:8.0` en `keygo-run/pom.xml`; Micrometer autotime activado; 9 tests unitarios `RequestTracingFilterTest`; plan en `docs/design/TRACING_TELEMETRY.md` |
