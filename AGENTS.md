...
- `http://localhost:8080/keygo-server/api/v1/platform/billing/subscription/cancel` (POST — **KEYGO_ADMIN / KEYGO_TENANT_ADMIN** — marcar cancelación al fin del período)
- `http://localhost:8080/keygo-server/api/v1/platform/billing/invoices` (GET — **KEYGO_ADMIN / KEYGO_TENANT_ADMIN** — facturas del contractor de plataforma)
...
- `http://localhost:8080/keygo-server/api/v1/tenants/{slug}/account/reset-password` (POST — **público** — resetear contraseña para usuario en estado `RESET_PASSWORD`; body: `request_id` UUID + `temporary_password` + `new_password` + `confirm_new_password` + `verification_code`; `request_id` viene en el `data.reset_code_id` del 401 del login bloqueado)
